...
export default class ComponentSearchBusiness extends React.Component {
    ...
    componentDidMount() {
        ...
        // for static data --
        // this.setState({
        //     businessList: businessSearchResult.businesses.sort((a,b) => { return b.rating-a.rating }).slice(0,5).map(business => {
    ...
    getData = (props) => {
        axios.get(`https://api.yelp.com/v3/businesses/search?term=${this.props.term}&location=${this.props.location}&categories=${this.props.categories}`, {
            headers: {
        ...
        .then(response => {
            this.setState({
                businessList: response.data.businesses.sort((a,b) => { return b.rating-a.rating }).slice(0,5).map(business => {
                    return <div key={business.id}>
                        <figure>
                            <img src={business.image_url} alt={business.alias} height="300" width="300" />
                            <figcaption>
